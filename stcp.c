...
char *stateName(int state) {
    return state == STCP_LISTEN ? "listen" : state == STCP_ESTABLISHED ? "established" : state == STCP_CLOSED ? "closed" :
        state == STCP_TIME_WAIT ? "time_wait" : state == STCP_TIME_WAIT_2 ? "time_wait_2" : "bogus state";
}
